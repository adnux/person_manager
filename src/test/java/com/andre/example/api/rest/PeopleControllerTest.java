...
import com.andre.example.api.rest.PeopleController;
...
public class PeopleControllerTest {
    ...
    public void greaterThanZero() throws Exception {
        mvc.perform(get("/api/people").accept(MediaType.APPLICATION_JSON))
    ...
    public void findByDocument() throws Exception {
        mvc.perform(get("/api/people?document=05169407912").accept(MediaType.APPLICATION_JSON))
                ...
                .andExpect(jsonPath("content[*].birth[0]", hasItem(1986)))
                .andExpect(jsonPath("content[*].birth[1]", hasItem(8)))
                .andExpect(jsonPath("content[*].birth[2]", hasItem(6)));
    }
    ...
    public void findOneById() throws Exception {
        mvc.perform(get("/api/people/59db44c59756ed0cb2103bfd").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.document", is("08782529329")))
                .andExpect(jsonPath("$.name", is("Antonio Carlos")))
                .andExpect(jsonPath("$.email", is("antonio.carlos@gmail.com")))
                .andExpect(jsonPath("$.birth[0]", is(1817)))
                .andExpect(jsonPath("$.birth[1]", is(10)))
                .andExpect(jsonPath("$.birth[2]", is(10)));
    }
    ...
    public void shouldCreateAndUpdateAndDelete() throws Exception {
        ...
        result = mvc.perform(put("/api/people/" + id)
                .accept(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON)
                .content(p1UpJson))
        ...
        // RETRIEVE updated
        mvc.perform(get("/api/people/" + id).accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$.birth[0]", is(2017)))
                .andExpect(jsonPath("$.birth[1]", is(10)))
                .andExpect(jsonPath("$.birth[2]", is(9)));
